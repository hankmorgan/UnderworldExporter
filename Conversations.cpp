				...
				switch (OPCode)
					{
						...
						case cnv_JMP: fprintf(LOGFILE, "JMP "); z = z + 2; fprintf(LOGFILE, " %d\n", (short)getValAtAddress(cnv_ark, func_ptr + z, 16)); counter++; break;
...
void ExtractConversationsUW2(int game)
	...
	/*
	if ((file = fopen("c:\\games\\uw\\save2\\bglobals.dat", "rb")) == NULL)
	fprintf(LOGFILE, "Could not open specified file\n");
	else
	{//Read in the game globals in the specified save file.
	fileSize = getFileSize(file);
	...
	*/
	...
	if ((file = fopen(UW2_CONVERSATION, "rb")) == NULL)
		fprintf(LOGFILE, "Could not open specified file\n");
	else
		fprintf(LOGFILE, "");
	fileSize = getFileSize(file);
	...
	fprintf(LOGFILE, "Conversation Header\nNo of Conversations %d", getValAtAddress(tmp_ark, 0, 16));
	int noOfConversations = getValAtAddress(tmp_ark, 0, 16);
	//add_ptr = 2;
	int address_pointer = 2;
	for (int i = 0; i < noOfConversations; i++)
		{
		int compressionFlag = getValAtAddress(tmp_ark, address_pointer + (noOfConversations * 4), 32);
		int isCompressed = (compressionFlag >> 1) & 0x01;
		long add_ptr = getValAtAddress(tmp_ark, address_pointer, 32);
		//	cnv_ptr = getValAtAddress(cnv_ark, add_ptr, 32);
		if (add_ptr != 0)
			{
			int datalen = 0;
			unsigned char *cnv_ark = unpackUW2(tmp_ark, add_ptr, &datalen);
			add_ptr = 0;
